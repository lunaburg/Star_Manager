...
- `apps/docs/mod_manage/scene_card_parsing.md`: StudioNEOV2 scene-card payload layout, scene-level UniversalAutoResolver map/item/pattern dependencies, local/remote zipmod and item matching (including Studio-domain items for scene dependencies), remote completion, and the detail-page related-tab display. Consult when changing scene-card parsing, scene dependency presentation, or remote completion.
...
- `apps/docs/mod_manage/standard_mod_structure_record.md`: standard zipmod archive layout, manifest/CSV relationships, recursive Studio registration tables including author-tool/Japanese headers, Unity3D references, thumbnail sources, and external `.zip` normalization. Consult when changing scanners, imports, or write-back behavior.
