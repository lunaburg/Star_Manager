...
- `apps/docs/trash-recycle-bin.md`: 人物卡与 zipmod 的 runtime/trash 回收站目录、删除、恢复、永久删除和索引恢复边界。修改删除或恢复流程时先查阅。
...
- `apps/docs/frontend-ui/main-layout.md`: application shell, global navigation, page-specific top bar visibility including the card-management exception, workspace regions, backend context, and global rebuild-database behavior.
