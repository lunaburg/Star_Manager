...
角色卡库用于浏览所选 HS2 游戏目录中的 AIS 人物卡和服装卡。该模块在导航中显示为“卡片管理”，侧栏的三个 SVG 类型按钮可互斥切换人物卡、服装卡和场景卡子界面。人物卡和服装卡已接入独立浏览、缩略图和详情接口；场景卡仍只显示工作区说明。文档内仍以“角色卡库”描述人物卡业务边界。

### 人物卡浏览器刷新按钮适配透明玻璃样式记录（2026-09-13）

- 背景：人物卡浏览器标题栏右侧的刷新按钮使用深色粗边框和实色背景，与当前页面的透明磨砂玻璃层不一致。
- 根因：按钮继承通用 `.module-icon-button` 的 3px 深色边框、6px 圆角和实色表面；该通用类同时被服装卡刷新按钮使用，不能直接全局改写。
- 解决方案：仅在 `.character-layout` 范围内为 `.character-refresh-button` 使用共享玻璃变量、半透明表面、10px 背景模糊、细白边和轻微内沿高光；悬停、聚焦和禁用旋转状态保持原有交互语义。
- 验证结果：通过 `npm run build` 与 `git diff --check` 验证；刷新调用、按钮尺寸、禁用条件和加载旋转动画保持不变。
- 适用边界：仅影响人物卡浏览器标题栏右侧刷新按钮，不改变服装卡刷新按钮、人物卡工具栏控件或其他页面的图标按钮。
...
- **场景卡**：切换到场景卡独立浏览工作区，资源根目录显示为 `UserData/studio/scene`。

激活类型由单一的前端 `cardBrowserMode` 状态维护，因此任意时刻只有一个按钮处于激活状态。服装卡使用独立的目录扫描和只读详情链路，不复用人物卡 SQLite 索引；场景卡暂未接入文件扫描。
...
服装卡列表按当前目录读取 PNG 文件项并每行显示 6 张卡片。进入目录时使用独立的 `runtime/clothes_card_index.sqlite` 增量索引：只有文件大小、纳秒修改时间和解析器版本都匹配且被确认是 `AIS_Clothes` 的文件才会显示；普通 PNG 会被记录为无效并始终过滤。首次遇到的文件只在当前页面附近做轻量信封校验，单次最多校验 96 个候选，扫描结果会持久化，后续打开不重复解析。索引完成后，列表查询直接由 SQLite 按文件名执行 `LIMIT/OFFSET` 分页，避免每次滚动都在 Python 中重建并排序整个目录。卡片名称优先使用索引中读取到的名称，缩略图由 `LazyThumbnail` 按当前视口及前方少量行请求，避免一次性解码大目录中的全部图片。卡片网格使用虚拟渲染，只创建可视区域前后少量行的 DOM；首批最多返回 96 张卡片，首屏附近的缩略图优先加载，继续滚动到列表底部时再按页追加。卡片沿用人物卡的 `63:88` 纵向比例并随网格宽度等比缩小，卡片内缩略图使用 `contain` 显示，底部保留粉橙渐变名称条；部件、配饰统计不在详情侧栏展示。目录树仍只做目录项和 PNG 候选数量的快速统计，直接显示数量；候选数量的含义保留在悬停提示中。点击某张卡后，才对这一张执行完整 `AIS_Clothes`、Coordinate、KKEx 和 UniversalAutoResolver 解析，并用解析出的名称和关联信息更新详情。缩略图缓存仍复用 `card_previews`，图片接口不负责把普通 PNG 放入列表，详情接口继续作为最终完整校验入口。侧栏沿用人物卡的左上角“目录 / 详情”tab：点击卡片自动切换到详情，目录树和详情内容一次只显示一个面板。目录树整行选择目录，展开按钮独立处理展开/收起。
...
目录和文件解析均限制在 `UserData/coordinate` 内；普通 PNG、人物卡路径和目录越界路径都会被过滤或拒绝。当前页面不提供服装卡写回、移动、删除或依赖提取操作。
...
- 验证：前端生产构建通过；599 张服装卡时每张卡均保留完整纵向卡面和滚动列表，少量卡片与窄窗口布局仍保持 6 列和可见卡面。
...
- 验证：前端生产构建通过；服装卡仍保持 `252:352` 卡片比例、六列网格、分页加载和缩略图懒加载。
