...
- `GET /library/cards/missing-mods?game_dir=&path=`
  ...
  - Returns GUID groups split into `available`, `unavailable`, and `local_item_missing`. An `available` group has remote candidates and can be installed; an `unavailable` group has no valid remote manifest; `local_item_missing` means the local zipmod exists but its specific item is not indexed, so it is shown for diagnosis and cannot be installed through this endpoint.
...
- `GET /library/clothes/missing-mods?game_dir=&path=`
  - Resolves one AIS clothes card's unresolved GUID dependencies against the same read-only remote index used by character cards. The result uses the same `available`, `unavailable`, and `local_item_missing` group states and never offers a download for a local zipmod whose specific item is missing.
...
- `GET /library/scene/missing-mods?game_dir=&path=`
  - Compares one scene card's unresolved map/item/pattern GUID dependencies with the read-only remote index at `backend/runtime/remote/remote_zipmod_index.sqlite`. It returns GUID groups split into `available`, `unavailable`, and `local_item_missing`, including usage counts and scene dependency locations. A remote candidate means the manifest GUID is available; after installation, the scene detail endpoint rechecks the exact item/pattern `Slot` and `LocalSlot` against the local index. A local zipmod with a missing scene item remains `local_item_missing` and is not offered for download.
