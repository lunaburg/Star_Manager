...
- 服装必须显示具体栏位名；配饰必须保留实际角色槽位映射。空配饰槽（`CategoryNo 350`）需要先指定 13 个饰品部位之一，再按该部位筛选和提交。面部和身体栏位都可以读取、筛选和提交对应物品换装；身体彩绘需要额外选择彩绘层。
