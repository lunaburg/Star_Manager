...
- `apps/scripts/python.cjs` explicitly includes `pyexpat`, `xml.parsers.expat`, `PIL`, `UnityPy`, `fmod_toolkit`'s `fmod.dll`, and available Conda runtime DLLs needed by XML parsing, compressed standard-library modules, and Unity3D thumbnail extraction. This avoids packaged-backend failures such as `No module named expat; use SimpleXMLTreeBuilder instead` while reading `manifest.xml`, `UnityPy is not available`, and frozen-runtime `fmod.dll` errors while extracting thumbnails.
