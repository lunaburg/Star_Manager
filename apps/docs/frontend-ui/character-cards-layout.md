...
- 展示规则：场景物品/图案未匹配本地物品数据库时，关联页按 ModID 合并为“模组”条目，并显示该模组下的缺失记录数量；已匹配的场景物品进入“工作室物品”分组，场景图案和场景地图进入“其它”分组；人物卡和服装卡继续按物品条目展示。
...
### 人物卡浏览器批量工具提示遮挡修复（2026-09-17）

- 背景：人物卡浏览器在多选状态下，移动、批量标签、删除和提取按钮的悬停提示向下展开；当筛选控件换行时，提示下半部分会被下一行控件覆盖。
- 根因：提示层与下一行筛选控件共处工具栏内，向下展开会落入后续控件的绘制区域。
- 解决方案：仅将人物卡批量图标按钮的提示和指示箭头改为向上展开，并提高该提示层级；通用图标提示及按钮交互保持不变。
- 验证结果：`npm run build` 通过；批量操作的鼠标悬停和键盘聚焦提示均不再与筛选控件重叠。
- 适用边界：仅影响人物卡多选工具栏的四个图标按钮，不改变服装卡、场景卡、模组管理或其他通用图标提示的位置。
...
- 关联项未精确匹配到数据库物品时，界面按 manifest GUID 合并为模组级条目，并查询远程索引。远程候选存在时显示“模组未安装 · 可补全”并提供“安装”；没有有效候选时显示“模组未安装 不可补全”。如果本地数据库已有该模组但缺少具体物品，则显示“模组存在，物品缺失”，不提供下载按钮。关联区域顶部在存在可下载缺失依赖时显示“全部安装”，可一次安装当前人物卡全部可获取的远程模组；未开始时保留单项“安装”按钮，任务开始后移除该按钮，在卡片进度栏内显示双状态 SVG 控件：下载中显示暂停图标，暂停后显示继续图标；下载任务运行期间，原“检查中”位置替换为“取消下载”，取消后等待后台确认并清理未完成的临时文件。每个正在处理的依赖卡片操作区域内显示阶段进度：下载阶段显示下载进度与累计已下载数据量，安装阶段只显示安装进度；空间不足时隐藏来源等次要文字，优先保证进度信息可读。批量补全最多同时下载 3 个文件，所有文件下载完成后才进入按序安装；SQLite 写入和人物卡依赖重连仍按安全的单文件顺序处理。
- 下载文件统一放入当前游戏目录的 `mods/Remote/`，保留远程索引的相对目录层级。后端先写 `.part` 临时文件并校验文件大小、ZIP CRC 和 manifest GUID，成功后原子改名；下载完成会执行单文件索引并重新读取当前卡片依赖状态。该补全流程同样用于服装卡和场景卡详情页。

### 三类卡片统一依赖状态

人物卡、服装卡和场景卡的关联列表使用同一套降级和补全规则：

- 数据库精确找到物品时显示物品级依赖和“已匹配”；
- 找不到物品时按 manifest GUID 合并为模组级依赖；
- 模组未安装且远端索引有有效候选时直接显示安装操作，可单项或全部安装；
- 模组未安装且远端没有有效候选时显示“模组未安装 不可补全”；
- 模组已安装但物品记录缺失时分两行显示“模组存在”和“物品缺失”，这是特殊诊断状态，不进入可补全列表。

模组级条目会保留原始依赖的使用位置和数量，安装后仍以本地数据库的物品级精确匹配结果为准。

模组级降级条目没有可对应的具体物品，因此不渲染物品缩略图或 `MISS` 占位块，也不重复显示 GUID/数量来源文字；条目在类型标签后显示“包含（N）个缺失物品”，布局直接使用模组名称和右侧状态/操作区域。状态徽标作为唯一状态表达，可补全时显示安装按钮，不可补全时不再额外显示重复控件；“模组存在”和“物品缺失”固定分两行显示并居中对齐。

当模组级条目对应的 zipmod 已存在于本地数据库时，点击该条目复用已有的模组定位流程，优先按本地 `zipmod_id` 查询并跳转到模组浏览器，随后选中对应模组；如果数据库中已没有该记录，则回退到缺失依赖提示。

缺失模组依赖统一收纳到“模组依赖缺失”分组；已匹配的 Studio 物品收纳到“工作室物品”分组，Studio 图案和地图收纳到“其它”分组，避免已存在的 Studio 物品被误报为缺失依赖。
...
- 验证：`apps/backend/tests/test_remote_mod_completion.py` 覆盖 3 路并发启动、坏包清理、manifest GUID 校验和已有文件重建索引；阶段回调验证下载阶段先于安装阶段，下载速度与字节统计字段按任务进度传递，前端下载阶段显示累计已下载数据量。
...
- 已收藏的人物卡网格使用静态暖橙色边框和不透明暖色名字铭牌表示状态，网格不叠加右上角星标；筛选器提供“正常”，用于查看无依赖缺失的人物卡；
...
- 按“模组依赖缺失、工作室物品、其它、服装、配饰、面部与五官、发型、身体与肌肤”的固定顺序分组；所有缺失的模组依赖集中到“模组依赖缺失”，已匹配的 Studio 物品进入“工作室物品”，Studio 图案和地图进入“其它”；
...
- 标题栏右侧提供“当前目录 / 人物卡库”标签筛选范围切换和当前目录刷新按钮；范围切换位于刷新按钮左侧。刷新先比较当前目录人物卡的新增、删除和文件大小/纳秒修改时间变化，发现变化后提交增量人物卡扫描任务，再重新读取当前列表和目录计数；没有变化时只重新读取列表，不改变目录选择和筛选范围。除范围切换和刷新外不放视图密度、更多操作等小按钮，避免和顶部全局按钮、下方工具栏重复。
...
- 基础筛选：可在当前目录的全部人物卡、正常人物卡和存在依赖缺失的人物卡之间切换；“正常”只显示无依赖缺失的人物卡，缺失卡片显示缺失依赖数量，切换筛选时清空批量选择。
- 收藏筛选：基础筛选左侧提供双态“收藏”按钮；激活时仅显示当前基础筛选结果中的收藏人物卡，非激活时不增加收藏条件，切换时清空批量选择和当前详情。
- 标签筛选：基础筛选旁提供独立的“搜索标签”输入框；“当前目录 / 人物卡库”范围切换位于标题栏刷新按钮左侧。输入为空时显示“已有标签”标题，并用不同颜色描边的圆角标签胶囊自然换行展示全库所有标签；输入内容后按包含关系显示纵向联想结果。候选项来自 SQLite 标签缓存并在当前前端会话中复用，不逐张读取 PNG；“当前目录”只显示当前目录中带有该标签的人物卡，“人物卡库”递归显示 `UserData/chara` 全库中所有匹配人物卡并排除 `navi`，清除输入则恢复全部人物卡。

### 人物卡收藏双态筛选记录（2026-09-17）

- **背景**：人物卡浏览器需要在不改变全部、正常、依赖缺失和标签筛选的前提下，快速筛选收藏人物卡。
- **解决**：在工具栏增加双态“收藏”按钮；激活时将收藏条件叠加到当前基础筛选结果，非激活时保持原筛选结果，并通过星标填充、暖色玻璃底和 `aria-pressed` 标识激活状态。
- **验证结果**：通过 `npm run build` 与 `git diff --check` 验证；按钮可切换，收藏筛选与正常、依赖缺失及标签筛选可组合使用。
- **适用边界**：仅影响人物卡浏览器的前端筛选和控件显示，不改变收藏数据、依赖解析、标签索引或其他卡片类型。

### 人物卡正常筛选替换收藏筛选记录（2026-09-17）

- **背景**：人物卡浏览器筛选器中的“已收藏”无法直接查看依赖完整、可正常使用的人物卡。
- **解决**：将筛选选项改为“正常”，按 `missingCount === 0` 筛选无依赖缺失的人物卡，并同步更新数量提示与空状态文案；收藏状态展示和收藏写回保持不变。
- **验证结果**：通过 `npm run build` 与 `git diff --check` 验证；全部人物卡、正常人物卡、依赖缺失人物卡和标签筛选仍可切换。
- **适用边界**：仅调整人物卡浏览器该筛选项的含义和文案，不改变人物卡依赖解析、收藏数据或其他卡片类型筛选。

### 标签筛选范围切换移至标题栏记录（2026-09-17）

- **背景**：标签筛选范围切换与搜索、基础筛选共占工具栏横向空间，标题栏刷新按钮左侧则存在可用的同层级区域。
- **解决**：将“当前目录 / 人物卡库”切换器移至人物卡浏览器标题栏右侧，紧邻刷新按钮左侧；工具栏保留多选、标签搜索和基础筛选。
- **验证结果**：通过 `npm run build` 与 `git diff --check` 验证；范围切换状态、标签搜索结果、当前目录和全库筛选行为保持不变。
- **适用边界**：仅调整人物卡浏览器的控件位置，不改变服装卡、场景卡、刷新或标签筛选逻辑。
...
- 方案：服装卡详情按服装、配饰、面部与五官、发型、身体与肌肤和模组依赖缺失分组，显示分组数量、部位标签、物品缩略图、来源模组和“已匹配 / 物品未找到 / 模组未安装”状态；详情接口同时使用卡内的服装卡 UAR 字段与本地模组数据库做物品匹配。
