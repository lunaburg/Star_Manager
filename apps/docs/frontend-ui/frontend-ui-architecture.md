...
- 导航入口图标统一采用 24×24 视图盒、单色描边 SVG；图标不再使用独立色块背景，激活态仅通过前景色变化区分，以保持侧栏的简约一致性。
- 卡片管理主入口使用两张错位叠放卡片与内容线，直接表达卡片资源库含义；人物卡、服装卡、场景卡仍使用右侧互斥类型按钮区分。
- 回收站入口使用带盖垃圾桶轮廓，桶盖、桶身和内部竖线清晰分层，避免与普通文件夹或收纳盒图形混淆。
...
- 服装卡使用 `UserData/coordinate` 下的独立目录扫描和 `clothes_card_index.sqlite` 增量索引；场景卡使用 `UserData/studio/scene` 下的独立目录扫描、分页和虚拟网格，预览与详情采用 `320:180` 横向比例。
